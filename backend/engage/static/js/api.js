        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if ($('.all.active').length == 0) {
                    ...
                    data.all_serializer.forEach((item, _index) => {
                        var winners = "";
                        if (item.top_winners != null && item.top_winners.length > 0) {
                            item.top_winners.slice(0, 3).forEach((item, index) => {
                                var html = `
    ...
    return new Promise((_resolve, _reject) => {
        ...
        if (tournaments_has_next || str == 'prev') {
            ...
            $.ajax({
                ...
                success: function (data) {
                    console.log("jebna l tournments");
                    changeWidth();
                    ppp = ppp + 1;
                    rebeh();
                   // bati2();
...
function changeWidth() {
     console.log("changewidth was callled ");
     var progress = document.querySelector(".progress-done");
    // progress.style.width ='50%' ;

    var c =((ppp/2)*100).toString();
   
    progress.style.width =c+"%";
    /* progress.inner.Text = '${Math.ceil(}%'; */
}
                                        ...
                                        <li>
                                            <a href="/tournaments/${item.slug}">
                                                ...
                                                <div class="search-right">
                                                    ...
                                                    <p class="post-meta">
                                                        <i class="fas fa-clock"></i>
                                                        <span>${item.starts_in < 0
                                            ? "Past"
                                            : "Starts in: " + item.starts_in
                                        }</span>
                                                        <i class="fas fa-users"></i>
                                                        <span>${item.current_participants
